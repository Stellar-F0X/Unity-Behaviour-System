...
namespace TaskStreamer.Tool
{
    ...
    public class ServiceContainerPanel : VisualElement, IRefreshablePanel
    {
        ...
        /// _serviceList는 ServiceContainerPanel 클래스에서 관리되는 ServiceBase 클래스의 리스트로,
        /// 현재 활성화된 서비스들의 데이터를 보관합니다.
        ...
        {
            if (_serviceListView.itemsSource is not null && _serviceListView.itemsSource.Count > 0)
            {
        ...
        {
            if (element is not ServiceSectionPanel servicePanel || index < 0 || _serviceList.Count <= index)
            {
                return;
            }
            ...
            servicePanel.Setup(serviceName, service, fields);
        }
