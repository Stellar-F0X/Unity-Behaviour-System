...
namespace TaskStreamer.Tool
{
    ...
    internal abstract class NodeIndicatorBase
    {
        ...
        protected readonly NodeViewBase _nodeView;
        ...
        public virtual bool TryHighlight(float deltaTime)
        {
        ...
        {
            Assert.IsNotNull(_nodeView);
            
            Assert.IsNotNull(_nodeView.targetNode);
        ...
        {
            this.StartHighlightIfNeeded();

            this.OnHighlightUpdate(_remainingTime * _settings.durationReciprocal);

            this._remainingTime -= deltaTime;

            this.EndHighlightIfNeeded();
        }


        /// <summary> 노드가 새로 호출되었을 때 하이라이트를 시작합니다. </summary>
        private void StartHighlightIfNeeded()
        {
        ...
        /// <summary> 하이라이트 지속 시간이 종료되었을 때 하이라이트를 종료합니다. </summary>
        private void EndHighlightIfNeeded()
        {
        ...
        public virtual void ApplyBorderColorByState()
        {
            ...
            if (_nodeView.targetNode.callCount <= 0)
            {
                return;
            }
        ...
        /// <summary> 하이라이트가 시작될 때 호출되는 메서드로, 상태별로 오버라이드하여 특수 동작을 구현합니다. </summary>
        protected abstract void OnHighlightStart();


        /// <summary> 하이라이트 진행 상황을 업데이트하는 메서드. </summary>
