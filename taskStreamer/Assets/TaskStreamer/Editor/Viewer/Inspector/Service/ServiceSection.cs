...
namespace TaskStreamer.Tool
{
	...
	internal class ServiceSection : VisualElement
	{
		...
		{
			TSUIElementSettings.instance.ServiceSection.CloneTree(this);
			...
			_deleteButton.iconImage = TSUIElementSettings.instance.DeleteButtonImage;
			_deleteButton.enabledSelf = TSEditor.canEditGraph;

			_enableToggle.RegisterValueChangedCallback(this.UpdateServiceEnableState);
			_headerFoldout.RegisterValueChangedCallback(this.UpdateServiceExpansion);
		}
		...
		private VisualElement CreateFieldElement(VariableHandle handle)
		{
			switch (handle.initialValue)
			{
				...
				case BlackboardBasedCondition: return new ConditionsPanel(handle);

				default: return new ErrorField(handle.context);
			}
		}



		private void UpdateServiceExpansion(ChangeEvent<bool> evt)
		{
			service.isExpanded = evt.newValue;
		}
		...
		private void UpdateServiceEnableState(ChangeEvent<bool> evt)
		{
			...
			this.service.enable = evt.newValue;
