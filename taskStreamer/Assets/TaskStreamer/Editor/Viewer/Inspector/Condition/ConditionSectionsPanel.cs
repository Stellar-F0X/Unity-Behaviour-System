...
namespace TaskStreamer.Tool
{
	...
	internal class ConditionsPanel : VisualElement, IRefreshableField
	{
		/// BlackboardBasedCondition의 조건 목록을 UI로 표시하는 VisualElement 클래스입니다.
		public ConditionsPanel(VariableHandle fieldInfo)
		{
			TSUIElementSettings.instance.ConditionSectionsPanel.CloneTree(this);
			...
			_conditionAddButton.iconImage = TSUIElementSettings.instance.AddButtonImage;
		...
		private void InitializeConditionListView(VariableHandle fieldInfo)
		{
			...
			_conditionListView.makeItem = () => new ConditionSection();
		...
		{
			TaskGraphView graphView = TSEditor.Instance.taskGraphView;
			BindingWindow window = BindingWindowBuilder.GetBuilder("Conditions", false)
			                                           ...
			                                           .AddFactoryModule(
				                                           () => new ConditionScriptCreationFactoryModule(graphView, _bbCondition.guid, "New Condition"),
		...
		{
			ConditionSection conditionSection = element as ConditionSection;

			Debug.Assert(conditionSection is not null, "conditionField is null");
		...
		{
			Undo.RecordObject(TSEditor.Instance.graphAsset, "TaskStreamer (AddBBBasedCondition)");
		...
		/// <param name="variableSection">삭제 요청된 BBBasedConditionField의 참조입니다.</param>
		private void OnVariableDeleteRequested(ConditionSection variableSection)
		{
			int index = _conditionListView.itemsSource.IndexOf(variableSection.conditionValueValue);
			...
			Undo.RecordObject(TSEditor.Instance.graphAsset, "TaskStreamer (RemoveBBBasedCondition)");
