...
namespace TaskStreamer.Tool
{
    public abstract class NodeView : Node
    {
        ...
        protected readonly EdgeDictionary _connectionEdge = new EdgeDictionary();
        ...
        {
            _nodeTypeLabel.text = TaskStreamerUtility.ApplySpacing(targetNode.GetType().Name);
            ...
            {
                this.TrackPropertyValue(new SerializedObject(targetNode).FindProperty("m_Name"), this.ChangeNodeViewName);
            }
        }
        ...
        public void UpdateView(float deltaTime)
        {
            ...
            EditorSettings settings = TaskStreamerEditor.settings;
            ...
            if (this.UpdateNodeHighlightState(deltaTime, settings.nodeViewHighlightingDuration))
            {
                ...
                this.SetEdgeColor(this._connectionEdge, settings.edgeStatusLinearColor.Evaluate(progress));
            }
        }
        ...
        /// <param name="duration">하이라이트 효과 지속 시간</param>
        /// <returns>노드 시각적 업데이트가 필요한 경우 true, 그렇지 않으면 false</returns>
        private bool UpdateNodeHighlightState(float deltaTime, float duration)
        {
            //노드가 새로 호출되었는지 확인
            ...
            {
                //하이라이트 지속 시간 증가
                _highlightDuration = Mathf.Min(_highlightDuration + deltaTime, duration);
                //남은 하이라이트 시간 감소
        ...
        protected virtual void OnViewEffectStart() { }

        protected virtual void OnViewEffectEnd() { }
        ...
        //
        public abstract void SetEdgeColor(EdgeDictionary control, Color color);
